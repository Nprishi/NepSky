    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">{t('admin.payments')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="text-right">
              ...
              <p className="text-2xl font-bold text-blue-600">रू {totalRevenueNPR.toLocaleString()}</p>
            </div>
          </div>
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredPayments.map((payment) => (
                <tr key={payment.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{payment.booking?.passenger_name || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-xs text-gray-500">Rate: {payment.exchange_rate}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{payment.payment_method}</div>
                    {payment.payment_gateway && (
                      <div className="text-xs text-gray-500">{payment.payment_gateway}</div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
        ...
        {filteredPayments.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-sm text-gray-600">
              {language === 'en' ? 'No payments found' : 'कुनै भुक्तानी फेला परेन'}
            </p>
          </div>
        )}
      </div>
