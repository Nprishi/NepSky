...
import { Search, BookOpen, Eye, X, User, Plane, CreditCard, Calendar, MapPin, Clock } from 'lucide-react';
...
const BookingManagement = () => {
  ...
  const handleUpdateStatus = async (id: string, newStatus: string, booking: BookingData) => {
    // Check if trying to cancel without user request
    if (newStatus === 'cancelled' && !booking.cancelled_by_user) {
      alert('Cannot cancel booking! Only user can request cancellation from their side.');
      return;
    }

    // If cancelling with user request, deduct revenue
    if (newStatus === 'cancelled' && booking.cancelled_by_user) {
      console.log(`Cancelling booking ${booking.booking_reference}. Deducting $${booking.total_amount} from revenue.`);
    ...
    if (!error) {
      console.log(`Booking ${booking.booking_reference} status updated to ${newStatus}`);
      loadBookings();
      if (showDetailsModal) {
        closeDetailsModal();
      }
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-blue-600">{selectedBooking.booking_reference}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-600">Current Status</p>
                    <span className={`px-3 py-1 rounded-full text-sm font-semibold ${getBookingStatusColor(selectedBooking.status)}`}>
                ...
                <div className="border rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium">{selectedBooking.flight.flight_number}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Airline</p>
                      <p className="font-medium">{selectedBooking.flight.airline}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Aircraft Type</p>
                      <p className="font-medium">{selectedBooking.flight.aircraft_type}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Available Seats</p>
                      <p className="font-medium">{selectedBooking.flight.available_seats}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex items-center justify-between bg-gray-50 rounded-lg p-4">
                    <div className="text-center">
                      ...
                      <p className="font-semibold text-lg">{selectedBooking.flight.from_location}</p>
                    ...
                    <div className="text-center">
                      ...
                      <p className="font-semibold text-lg">{selectedBooking.flight.to_location}</p>
                      <p className="text-sm text-gray-600 mt-1">
                        <Clock className="h-4 w-4 inline mr-1" />
              ...
              <div className="border rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getPaymentStatusColor(selectedBooking.payment_status)}`}>
                      {selectedBooking.payment_status}
                    </span>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Total Amount</p>
                    <p className="font-bold text-xl text-green-600">${Number(selectedBooking.total_amount).toFixed(2)}</p>
                  </div>
                  <div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Update Booking Status</h3>
                <div className="flex items-center space-x-4">
                  <select
                    value={selectedBooking.status}
                    onChange={(e) => handleUpdateStatus(selectedBooking.id, e.target.value, selectedBooking)}
                  ...
                  >
                    ...
                    <option value="cancelled" disabled={!selectedBooking.cancelled_by_user}>
                      Cancelled {!selectedBooking.cancelled_by_user && '(User must request)'}
                    </option>
                  </select>
                </div>
                {!selectedBooking.cancelled_by_user && selectedBooking.status !== 'pending_cancellation' && (
                  <p className="mt-2 text-sm text-yellow-700">
                    ⚠️ Cancellation can only be processed if the user requests it from their side.
                  </p>
                )}
                {selectedBooking.status === 'pending_cancellation' && (
                  <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                    ...
                    <ul className="text-sm text-red-700 list-disc list-inside space-y-1 mb-3">
                      ...
                      <li>Deduct ${Number(selectedBooking.total_amount).toFixed(2)} from total revenue</li>
                      <li>Free up seat: {selectedBooking.seat_number}</li>
                    </ul>
                    <button
                      onClick={() => handleUpdateStatus(selectedBooking.id, 'cancelled', selectedBooking)}
